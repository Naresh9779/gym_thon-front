...
Admin credentials are set via environment variables (ADMIN_EMAIL and ADMIN_PASSWORD).

**⚠️ IMPORTANT**: 
- Set a strong admin password in your environment variables
...
```env
# MongoDB Connection
MONGODB_URI=mongodb+srv://YOUR_MONGODB_USER:YOUR_MONGODB_PASSWORD@YOUR_CLUSTER.mongodb.net/YOUR_DATABASE?retryWrites=true&w=majority&appName=YOUR_APP_NAME
...
2. **Test Login**: 
   - Email: Use your ADMIN_EMAIL value
   - Password: Use your ADMIN_PASSWORD value
3. **Verify Features**:
   - ✅ Login works
...
| ADMIN_EMAIL | Auto-created admin email | ⚠️ Set in env vars |
| ADMIN_PASSWORD | Auto-created admin password | ⚠️ Set in env vars (use strong password!) |
...
**Admin Login**: Use credentials from your ADMIN_EMAIL and ADMIN_PASSWORD environment variables (change password immediately after first login!)
