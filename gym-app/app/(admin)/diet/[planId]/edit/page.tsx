...
export default function AdminEditDietPage({ params }: Props) {
  ...
  const [avgDailyCalories, setAvgDailyCalories] = useState<number | ''>('');
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        setPlan(p);
        setName(p.name || '');
        const rawDate = p.weekStartDate;
        setWeekStartDate(rawDate ? new Date(rawDate).toISOString().slice(0, 10) : '');
        setAvgDailyCalories(p.avgDailyCalories ?? '');
        setProtein(p.avgMacros?.protein ?? '');
        setCarbs(p.avgMacros?.carbs ?? '');
        setFats(p.avgMacros?.fats ?? '');
        setDays(p.days || []);
        setLoading(false);
      } catch (e: any) {
  ...
  const save = async () => {
    ...
    if (weekStartDate) body.weekStartDate = weekStartDate;
    if (typeof avgDailyCalories === 'number' && avgDailyCalories >= 0) body.avgDailyCalories = avgDailyCalories;
    const avgMacros: any = {};
    if (typeof protein === 'number') avgMacros.protein = protein;
    if (typeof carbs === 'number') avgMacros.carbs = carbs;
    if (typeof fats === 'number') avgMacros.fats = fats;
    if (Object.keys(avgMacros).length) body.avgMacros = avgMacros;
  ...
  const addMeal = () => {
    const updated = [...days];
    if (!updated[currentDayIdx]) return;
    updated[currentDayIdx] = {
      ...updated[currentDayIdx],
      meals: [...updated[currentDayIdx].meals, { time: '08:00', name: 'New Meal', foods: [], totalCalories: 0, macros: { protein: 0, carbs: 0, fats: 0 } }]
    };
    setDays(updated);
  };
  ...
  const removeMeal = (mi: number) => {
    ...
    updated[currentDayIdx] = {
      ...updated[currentDayIdx],
      meals: updated[currentDayIdx].meals.filter((_, idx) => idx !== mi)
    };
    setDays(updated);
  };
  ...
  const addFood = (mi: number) => {
    const updated = [...days];
    updated[currentDayIdx].meals[mi].foods.push({ name: '', portion: '', calories: 0, protein: 0, carbs: 0, fats: 0 });
    setDays([...updated]);
  };

  const removeFood = (mi: number, fi: number) => {
    const updated = [...days];
    updated[currentDayIdx].meals[mi].foods = updated[currentDayIdx].meals[mi].foods.filter((_, i) => i !== fi);
    recalcMeal(mi, updated);
  };

  const updateFood = (mi: number, fi: number, field: keyof FoodItem, value: any) => {
    const updated = [...days];
    updated[currentDayIdx].meals[mi].foods[fi] = { ...updated[currentDayIdx].meals[mi].foods[fi], [field]: value };
    recalcMeal(mi, updated);
  };

  const updateMeal = (mi: number, field: 'time' | 'name', value: string) => {
    const updated = [...days];
    updated[currentDayIdx].meals[mi][field] = value;
    setDays(updated);
  };

  const recalcMeal = (mi: number, arr: DietDay[]) => {
    const meal = arr[currentDayIdx].meals[mi];
    ...
    <div className="space-y-5 max-w-4xl">
      ...
      {section('Plan Details', 'Calories and macro targets', (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="label-cap block mb-2">Week Start Date</label>
            <input type="date" value={weekStartDate} onChange={e => setWeekStartDate(e.target.value)} className={inputCls} />
          </div>
          <div>
            <label className="label-cap block mb-2">Avg Daily Calories <span className="text-gray-400 normal-case font-medium">(kcal)</span></label>
            <input type="number" value={avgDailyCalories as any} onChange={e => setAvgDailyCalories(Number(e.target.value))} className={inputCls} min="0" />
          </div>
      ...
      <div className="space-y-3">
        ...
        {days.length > 0 && (
          <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 px-4 py-3">
            ...
            <div className="text-center">
              <p className="text-xs font-bold text-gray-400">DAY {currentDayIdx + 1} OF {days.length}</p>
              <p className="font-black text-gray-900">
                {currentDay?.dayName || `Day ${currentDayIdx + 1}`}
                {currentDay?.date && (
                  <span className="text-gray-400 font-semibold text-sm ml-2">
                    {new Date(currentDay.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                  </span>
                )}
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-gray-50">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {days.length === 0 ? 'No days in plan' : `${currentDay?.meals?.length ?? 0} meal${(currentDay?.meals?.length ?? 0) !== 1 ? 's' : ''} on ${currentDay?.dayName || `Day ${currentDayIdx + 1}`}`}
              </p>
            </div>
            {days.length > 0 && (
              <button
          ...
          <div className="p-5 space-y-4">
            {days.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-6">No days in this plan.</p>
            ) : !currentDay?.meals?.length ? (
              <p className="text-sm text-gray-400 text-center py-6">No meals for {currentDay?.dayName || `Day ${currentDayIdx + 1}`}. Click "Add Meal" to create one.</p>
